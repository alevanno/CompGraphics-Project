...
vec4 lightModel(int lt, vec3 pos, vec3 lPos) {
...
void main() { 
	...
	vec4 lm = lightModel(lightType, fsPosition, lightPosition);
	...
	if(lightType == 5){
		...
		vec4 diffuse = diffuseTextureColorMixture * lightColor * clamp(dot(lDir,nNormal), 0.0, 1.0) * lightDimension;	
		
		//Reflection vector for Phong model
		vec3 reflection = -reflect(nlightDirection, nNormal);
		//Specular not used, since it give an unreallistic effect	
		//vec4 specular = mSpecColor * lightColor * pow(clamp(dot(nEyeDirection, reflection),0.0, 1.0), mSpecPower) * lightDimension;
		gl_FragColor = ambLight + diffuse ; 
		//dovrei rifare il calcolo di ogni gl_fragColor per ogni tipo di light
		//salvando però il risultato in light1, e così via dicendo fino a light8
		//e poi fare la seguente formula
		//gl_fragColor = clamp(light1 + ... + light8 + amblight, 0.0, 1.0);		
	}
