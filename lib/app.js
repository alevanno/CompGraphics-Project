...
var dirLightAlpha = -utils.degToRad(80); // around x axis
var dirLightBeta  = -utils.degToRad(0); // around z axis
...
//1s 4 lights use lightdirection1

var lightPosition = [[0.0, 0.0, -10.0], [-0.0, 0.0, -1.3], [10.0, 10.0, 10.0], [10.0, 10.0, 10.0],
...
function objectOrientation(objectWorldMatrix, sceneObjects) {
    ...
    switch (sceneObjects) {
        ...
        case bedObj: // bed
            objectWorldMatrix[sceneObjects] = utils.multiplyMatrices(
                objectWorldMatrix[sceneObjects],
                utils.MakeScaleMatrix(0.6));
            objectWorldMatrix[sceneObjects] = utils.multiplyMatrices(
                objectWorldMatrix[sceneObjects],
                utils.MakeRotateYMatrix(-90));
            objectWorldMatrix[sceneObjects] = utils.multiplyMatrices(
                objectWorldMatrix[sceneObjects],
                utils.MakeTranslateMatrix(-3.0, 0.0, -2.5));
            break;
        case chairObj: // chair
            objectWorldMatrix[sceneObjects] = utils.multiplyMatrices(
                objectWorldMatrix[sceneObjects],
            ...
            break;
        case closetObj0: // closet
        case closetObj1: // closet
        case closetObj2: // closet
        case closetObj3: // closet
        case closetObj4: // closet
            objectWorldMatrix[sceneObjects] = utils.multiplyMatrices(
                objectWorldMatrix[sceneObjects],
                utils.MakeTranslateMatrix(1.0, 0.0, 1.5));
            objectWorldMatrix[sceneObjects] = utils.multiplyMatrices(
                objectWorldMatrix[sceneObjects],
    ...
    console.log(objectWorldMatrix[sceneObjects]);
}
...
//var room = [[-1.8, -1.6, 1.8, 1.6]]

//TODO remove or adapt
var zones = [[], [], [], [], [],
    [-0.7, -2.0, 2.0, -0.5], [-2.0, 2.0, 0.5, -0.0], [-2.0, -2.0, -0.5, -0.5], [-1.72, 1.5, -2.7, 3.0], [-1.0, 1.5, -1.71, 3.0], [0.8, 1.5, -0.4, 3.0]];
function popup(x, y) {
    var inZone = 0;
    var i;
